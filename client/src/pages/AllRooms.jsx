                ...
                {filteredRooms.map((room) => (
                    ...
                    border-b border-gray-300 last:pb-30 last:border-0'>
                        <img onClick={() => { navigate(`/rooms/${room._id}`); scrollTo(0, 0) }}
                            src={room.images[0]} alt="hotel-img" title='views Room Details'
                    ...
                    <div className='px-5 pt-5'>
                        ...
                        {roomTypes.map((room, index) => (
                            <CheckBox key={index} label={room} selected={selectedFilters.roomType.includes(room)}
                                onChange={(checked) => handleFiltersChange(checked, room, 'roomType')} />
                        ))}
                    </div>
                    <div className='px-5 pt-5'>
                        ...
                        {priceRanges.map((range, index) => (
                            <CheckBox key={index} label={`${currency} ${range}`} selected={selectedFilters.priceRange.includes(range)}
                                onChange={(checked) => handleFiltersChange(checked, range, 'priceRange')} />
                        ))}
                    </div>
